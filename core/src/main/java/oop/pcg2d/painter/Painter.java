                        ...
                        if (is_bot(x, i, j)) {
                            if (is_left(x, i, j)) {
                                if (is_right(x, i, j)) {     // 상, 하, 좌, 우 블록 존재할때
                                    batch.draw(image[4], j * 32, (x.length - i - 1) * 32, 32, 32);
                                } else {        // 상, 하, 좌 블록 존재할때
                                    batch.draw(image[5], j * 32, (x.length - i - 1) * 32, 32, 32);
                                }
                            } else {
                                if (is_right(x, i, j)) {  // 상, 하, 우 블록 존재할때
                                    batch.draw(image[3], j * 32, (x.length - i - 1) * 32, 32, 32);
                                } else {   // 상, 하
                                    batch.draw(image[5], j * 32, (x.length - i - 1) * 32, 32, 32);
                                }
                            }
                            ...
                            if (is_left(x, i, j)) {
                                if (is_right(x, i, j)) {      // 상, 좌, 우
                                    batch.draw(image[7], j * 32, (x.length - i - 1) * 32, 32, 32);
                                } else {        // 상, 좌
                                    batch.draw(image[8], j * 32, (x.length - i - 1) * 32, 32, 32);
                                }
                            } else {
                                if (is_right(x, i, j)) {        // 상, 우
                                    batch.draw(image[6], j * 32, (x.length - i - 1) * 32, 32, 32);
                                } else {       // 상
                                    batch.draw(image[7], j * 32, (x.length - i - 1) * 32, 32, 32);
                                }
                            }
                        ...
                        if (is_bot(x, i, j)) {
                            if (is_left(x, i, j)) {
                                if(is_right(x, i, j)) {     // 하, 좌, 우
                                    batch.draw(image[1], j * 32, (x.length - i - 1) * 32, 32, 32);
                                } else {        // 하, 좌
                                    batch.draw(image[2], j * 32, (x.length - i - 1) * 32, 32, 32);
                                }
                            }
                            else {
                                if(is_right(x, i, j)) {     // 하, 우
                                    batch.draw(image[0], j * 32, (x.length - i - 1) * 32, 32, 32);
                                } else {        // 하
                                    batch.draw(image[1], j * 32, (x.length - i - 1) * 32, 32, 32);
                                }
                            }
